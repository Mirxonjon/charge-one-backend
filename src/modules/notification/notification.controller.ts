...
export class NotificationController {
  constructor(private service: NotificationService, private jwt: JwtService) {}
  ...
  async removeDevice(@Req() req: any, @Headers('device-token') deviceTokenHeader: string, @Body('deviceToken') deviceTokenBody?: string) {
    const userId = this.getUserIdFromReq(req);
  ...
  async broadcast(@Body() dto: BroadcastDto) {
    return this.service.broadcast({ title: dto.title, body: dto.body, type: dto.type, data: dto.data });
  }
